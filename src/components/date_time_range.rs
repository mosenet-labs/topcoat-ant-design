...
pub async fn date_time_range_filter(
    ...
    Ok(view! {
        <span (attrs)>
            ...
            <aside id=(id) class="gr-date-range-panel" popover="auto" role="dialog" aria-label="选择时间范围" style=(panel_anchor)>
                <div class="gr-date-range-fields">
                    <label for=(from_id.as_str())><span>"开始时间"</span><input id=(from_id.as_str()) name=(from_name) type="datetime-local" value=(from)></label>
                    <span class="gr-date-range-separator" aria-hidden="true">"→"</span>
                    <label for=(to_id.as_str())><span>"结束时间"</span><input id=(to_id.as_str()) name=(to_name) type="datetime-local" value=(to)></label>
                </div>
                <div class="gr-date-range-quick" aria-label="快捷时间范围">
                    <button type="button" (quick_day)>"最近 24 小时"</button>
                    <button type="button" (quick_week)>"最近 7 天"</button>
                    <button type="button" (quick_month)>"最近 30 天"</button>
                </div>
                <footer class="gr-date-range-actions">
                    ...
                    <button class="gr-button gr-button-primary" type="submit">"应用"</button>
                </footer>
            </aside>
        </span>
    })
...
fn range_label(from: &str, to: &str) -> String {
    match (from.is_empty(), to.is_empty()) {
        (true, true) => "全部时间".to_owned(),
        (false, true) => format!("{} 起", compact_time(from)),
        (true, false) => format!("截至 {}", compact_time(to)),
...
mod tests {
    ...
    fn labels_empty_and_complete_ranges() {
        assert_eq!(range_label("", ""), "全部时间");
        assert_eq!(
            range_label("2026-09-20T10:00", "2026-09-21T10:00"),
