...
pub async fn notification(
    ...
    let notification_class = class!(
        "native-ui gr-notification group relative grid min-h-[92px] grid-cols-[32px_minmax(0,1fr)] gap-3 overflow-hidden rounded-lg border border-[#f0f0f0] bg-white px-[18px] pb-4 pt-[18px] font-mono text-[#262626] shadow-lg pointer-events-auto animate-[gr-notification-enter_180ms_ease-out]",
        tone.class_name(),
        caller_class,
    );
    ...
    Ok(view! {
        <div id=(popover_id.as_str()) popover="manual" class="pointer-events-none fixed bottom-auto left-auto m-0 right-6 top-6 z-[1300] w-[min(400px,calc(100vw_-_32px))] overflow-visible border-0 bg-transparent p-0 max-[600px]:right-4 max-[600px]:top-4"
            ...
            <article (attrs)>
                ...
                button::button(variant: button::ButtonVariant::Ghost, size: button::ButtonSize::Icon, attrs: attributes! { cx => class="absolute right-[18px] top-[18px] size-[22px] rounded-[4px] p-0 text-[rgba(0,0,0,0.45)] hover:text-[rgba(0,0,0,0.88)]" type="button" aria-label=(language.select("Close notification", "关闭通知")) @click=$(|_e: Event| message.set("".to_owned())) },
