        ...
        <div class="grid gap-6">
            component_example(id: "popconfirm-preview", title: text(locale, "组件预览"), description: text(locale, "确认后气泡立即恢复隐藏状态，业务回调继续执行。"), source: example_source,
                <div class="grid grid-cols-2 gap-4 p-6 max-[680px]:grid-cols-1">
                    <article class="rounded-lg border border-[#edf0f4] bg-[#fafafa] p-5"><h3 class="m-0 text-base font-semibold">(text(locale, "普通操作"))</h3><p class="mb-5 mt-2 min-h-11 text-sm leading-6 text-[#595959]">(text(locale, "适用于启用、重新验证等可恢复操作。"))</p><button class="cursor-pointer border-0 bg-transparent p-0 font-mono text-sm text-[#1677ff] hover:underline" type="button" (normal_trigger)>(text(locale, "启用实例"))</button>
                    ...
                    <article class="rounded-lg border border-[#edf0f4] bg-[#fafafa] p-5"><h3 class="m-0 text-base font-semibold">(text(locale, "危险操作"))</h3><p class="mb-5 mt-2 min-h-11 text-sm leading-6 text-[#595959]">(text(locale, "危险样式由调用方的确认按钮决定。"))</p><button class="cursor-pointer border-0 bg-transparent p-0 font-mono text-sm text-[#ff4d4f] hover:underline" type="button" (danger_trigger)>(text(locale, "停用实例"))</button>
                    ...
                    <article class="relative col-span-2 rounded-lg border border-[#edf0f4] bg-[#fafafa] p-5 max-[680px]:col-span-1"><div class="flex items-end justify-between gap-8"><div><h3 class="m-0 text-base font-semibold">(text(locale, "边缘偏移"))</h3><p class="mb-0 mt-2 text-sm leading-6 text-[#595959]">(text(locale, "气泡靠近视口边缘时保持可见，箭头继续指向触发按钮。"))</p></div><button class="shrink-0 cursor-pointer border-0 bg-transparent p-0 font-mono text-sm text-[#1677ff] hover:underline" type="button" (edge_trigger)>(text(locale, "边缘操作"))</button></div>
