...
async fn icon_sample(data: IconData, name: &str, description: &str) -> Result<impl View> {
    Ok(view! {
        <article class="flex min-h-32 flex-col justify-between rounded-lg border border-border bg-card p-4 transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-[var(--gr-accent-border)] hover:shadow-sm">
            <span class="text-[28px] text-primary">icon(data: data)</span>
            <div><strong class="mt-5 block text-sm font-semibold">(name)</strong><small class="mt-1 block text-xs leading-5 text-muted-foreground">(description)</small></div>
        </article>
    })
}
...
pub(in crate::app) async fn icons_page(cx: &topcoat::context::Cx) -> Result<impl View> {
    ...
    Ok(view! {
        ...
        <section class="mb-8 rounded-xl border border-border bg-card p-6 shadow-sm" aria-labelledby="icon-set-title">
            <h2 class="mb-2 mt-0 text-lg font-semibold" id="icon-set-title">(text(locale, "控制台图标集"))</h2>
            <p class="mb-5 mt-0 text-sm leading-6 text-muted-foreground">(text(locale, "图标继承文字颜色，默认跟随字号缩放；下列项目均直接使用 Topcoat icon 组件渲染。"))</p>
